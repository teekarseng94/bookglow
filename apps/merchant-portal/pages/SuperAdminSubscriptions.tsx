...
const readinessCopy: Record<BillingReadiness['state'], { label: string; detail: string; tone: 'success' | 'warning' | 'danger' | 'neutral' }> = {
  ...
  provider_not_configured: { label: 'Provider not configured', detail: 'Stripe server credentials are missing. Checkout and portal actions are disabled.', tone: 'warning' },
  configured_unverified: { label: 'Configured, unverified', detail: 'Stripe responded, but the recurring price or webhook endpoint could not be fully verified.', tone: 'warning' },
  readiness_verified: { label: 'Readiness verified', detail: 'Stripe price and webhook endpoint were verified by the billing backend.', tone: 'success' },
  billing_service_error: { label: 'Billing service error', detail: 'The provider is configured but its readiness check failed.', tone: 'danger' },
};
...
const SuperAdminSubscriptions: React.FC = () => {
  ...
  const openBilling = async (outletId: string, existing: boolean) => {
    ...
    try {
      const url = existing ? await platformOperationsService.createBillingPortal(outletId) : await platformOperationsService.createCheckout(outletId, import.meta.env.VITE_STRIPE_PRICE_ID);
  ...
  return <div className="space-y-5">
    <PlatformPageHeader title="Subscriptions" description="Bookglow subscription revenue from Stripe. Merchant sales are not included." meta={readiness?.checkedAt ? <span className="text-xs text-[var(--text-muted)]">Last checked {new Date(readiness.checkedAt).toLocaleString()}</span> : undefined} />
    ...
    <PlatformSection title="Outlet billing readiness" description="Merchant portal access is independent from Bookglow billing status.">
      ...
      {loading ? <LoadingSkeleton rows={6} className="p-4" /> : error ? <ErrorState className="m-4" message={error} onRetry={load} /> : filtered.length === 0 ? <EmptyState className="m-4" title="No outlets found" description="Try another outlet name or identifier." /> : <div className="divide-y divide-[var(--line)]">{filtered.map((outlet) => {
        ...
        const canAct = Boolean(readiness && (subscription ? readiness.portalReady : readiness.checkoutReady));
        return <div key={outlet.outletID} className="grid gap-3 px-4 py-3.5 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto] sm:items-center">
          <div className="min-w-0"><p className="truncate text-sm font-semibold text-[var(--text-primary)]">{outlet.name || outlet.settings?.shopName || outlet.outletID}</p><p className="mt-0.5 font-mono text-[10px] text-[var(--text-muted)]">{outlet.outletID}</p></div>
          ...
          <Button size="sm" variant="secondary" disabled={!canAct || busyOutlet === outlet.outletID} title={!canAct ? 'Billing prerequisites are not verified.' : undefined} onClick={() => void openBilling(outlet.outletID, Boolean(subscription))}>{busyOutlet === outlet.outletID ? 'Opening…' : subscription ? 'Manage billing' : 'Start subscription'}</Button>
        </div>;
      })}</div>}
    </PlatformSection>
  </div>;

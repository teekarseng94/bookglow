...
const SuperAdminAudit: React.FC = () => {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      auditService.getPage({ page, pageSize, search, action, actor, outletId, from: from ? new Date(`${from}T00:00:00`).toISOString() : undefined, to: to ? new Date(`${to}T23:59:59.999`).toISOString() : undefined })
  ...
  }, [page, search, action, actor, outletId, from, to]);
  ...
  return <div className="space-y-5">
    ...
    <PlatformSection title="Administrative activity" description={loading ? 'Loading authoritative records…' : `${events.length} records on this page · ${total} matching events`}>
      ...
      {loading ? <LoadingSkeleton rows={8} className="p-4" /> : error ? <ErrorState className="m-4" title="Server audit unavailable" message={error} /> : events.length === 0 ? <EmptyState className="m-4" title="No audit events found" description="No authoritative server events match these filters." /> : <div className="divide-y divide-[var(--line)]">{events.map((event) => <article key={event.id} className="grid gap-2 px-4 py-4 lg:grid-cols-[170px_minmax(0,1fr)_220px]">

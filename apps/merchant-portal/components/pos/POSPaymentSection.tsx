...
export const POSPaymentSection: React.FC<POSPaymentSectionProps> = ({
  ...
  <div className={cx('space-y-2 posd:space-y-3', className)}>
      ...
      {useCustomDateTime ? (
        <div className="flex gap-2">
          <input
            ...
            className="m-pos-control box-border min-h-[44px] flex-1 rounded-ui-md border border-[var(--line)] bg-[var(--bg-surface)] px-2.5 py-1.5 text-sm outline-none focus-visible:shadow-ui-focus-strong"
          />
          <input
            type="time"
            value={customTime}
            onChange={(e) => onCustomTimeChange(e.target.value)}
            className="m-pos-control box-border h-auto w-28 min-h-[44px] rounded-ui-md border border-[var(--line)] bg-[var(--bg-surface)] px-2.5 py-1.5 text-sm outline-none focus-visible:shadow-ui-focus-strong"
          />
        </div>
      ) : null}
    </div>
    ...
    {/* Payment method */}
    <div className="space-y-1.5">
      {/* Phone: compact row. Tablet+: stacked 44px select. */}
      <div className="flex min-w-0 items-center gap-2 post:hidden">
      ...
      {/* Tablet + desktop: stacked label + full-width select */}
      <div className="hidden space-y-1.5 post:block">
        <label className="m-pos-label uppercase tracking-wider text-[var(--text-muted)]">Payment Method</label>
        <select
          className="m-pos-control m-pos-payment-select box-border w-full min-h-[44px] rounded-ui-md border border-[var(--line)] bg-[var(--bg-surface)] px-2.5 py-2 text-sm font-semibold outline-none focus-visible:shadow-ui-focus-strong"
          value={paymentMethod}
          onChange={(e) => onPaymentMethodChange(e.target.value)}
          disabled={paymentDisabled}
          aria-label="Payment method"

/**
 * Point Transaction Service — dual provider (Firestore / Supabase Phase 2).
 */
...
const firestorePointTransactionService = {
  getAll: async (clientId: string, outletID: string = currentOutletID): Promise<PointTransaction[]> => {
    ...
    return snapshot.docs.map(
      (d) => ({ id: d.id, ...d.data() }) as PointTransaction
    );
  },
...
export const pointTransactionService = useSupabase()
  ? supabaseMerchant.pointTransactionService
  : firestorePointTransactionService;

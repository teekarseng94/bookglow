-- Member renewal: store latest renewal metadata on clients and atomically
-- create a SALE transaction (same shape as POS) via renew_member_membership.

ALTER TABLE public.clients
  ADD COLUMN IF NOT EXISTS last_renewed_at timestamptz NULL,
  ADD COLUMN IF NOT EXISTS last_renewal_amount numeric NULL;
...
REVOKE ALL ON FUNCTION public.renew_member_membership(text, numeric, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.renew_member_membership(text, numeric, text, text) TO authenticated;
